...
# VISIBLY pure_select_use_genes is not good,
# SO LETS just take pure_select_11

# m <- pure_select_use_genes[[1]]
# labels<-rep(1,dim(pure_select_use_genes[[1]])[[1]])
# for (i in 2:11) {
#     m<-rbind(m, pure_select_use_genes[[i]])
#     labels<-c(labels, rep(i,dim(pure_select_use_genes[[i]])[[1]]))
# }
...
write.svmlight(m, cls_id, file.path(RES_DIR,"68k_assignments.svmlight"))
