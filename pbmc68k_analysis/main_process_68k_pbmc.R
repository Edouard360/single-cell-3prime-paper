...
fig_inferred_labels<-ggplot(tdf_mod_2,aes(X1,X2,col=cls_id))+geom_point(size=0,alpha=1)+theme_classic()+.set_pbmc_color_11()
.save_figure(fig_inferred_labels,'./','pca','tsne',width=8.0,height=4)
# --------------------------------------------------
...
plot_heatmap_and_write_genes(n_clust=10, tsne_df=tdf_n_1000, mat=m_n_1000, genes=use_genes_n_id[1:1000],
                              dir=file.path(RES_DIR,"k10_marker_genes.tsv"), name="3c.png")
